...
#include "gzparallelgripperplugin/gzparallelgripperplugin.h"
    ...
    try {
        ...
        if(bCollisions)
        {
            ...
            for (unsigned int j = 0; j < this->gzlinks.size(); ++j)
            {
                ...
                if (bDebug) gzdbg << "Add collision = " << collName << "\n";
        ...
        // Fix...
        if (bDebug) gzdbg << "gzParallelGripperPlugi::Load setGravityMode\n";
        this->model->SetGravityMode(false);
        linear_velocity = ignition::math::Vector3d(0,0,0);
        angular_velocity = ignition::math::Vector3d(0,0,0);

        // setup up ROS
        if (bDebug)  gzdbg << "gzParallelGripperPlugi::Load setup up ROS\n";
        if (!ros::isInitialized())
        {
        ...
        if (bDebug) gzdbg << "gzParallelGripperPlugi::Load Create ROS node\n";
        // Create ROS node.
        ...
        if (bDebug) gzdbg << "gzParallelGripperPlugi::Load Done!\n";
